...
<html>
		...
		<div id="page">
			...
			<legend class="titre">Résultats</legend>
				...
				<fieldset class="sous-champ">
					...
					<?php displayArrayResults($arResults); ?>
				</fieldset>
			...
			<form action="controleur/downloadCSV.php" method="POST" TARGET=_BLANK style="display:inline-block;margin-left: 1em">
				<input type="hidden" value="<?php echo htmlentities(serialize($arResults)); ?>" name="csvString" />
	...
	$(document).ready( function () {
		...
		$("#sauver").click(function(){
			...
			var results = <?php echo json_encode($array_results); ?>;
			...
			if (<?php echo json_encode(isset($_SESSION['login'])); ?>) {
				...
				type : 'POST',
				data : {action:'save', results:results, table_name:table_name},
				dataType: 'html',
